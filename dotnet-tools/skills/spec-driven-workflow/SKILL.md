...
- [ ] **Define visual testing plan (playwright-cli):**
  - Identify target screens and user interaction scenarios to verify.
  - Define screenshot capture points for each key state (initial load, after user action, validation errors, success states, etc.).
...
- [ ] Execute visual testing with playwright-cli.
      - Start the application via .NET Aspire AppHost (`dotnet run --project SampleApp.AppHost`).
      - Open a headed browser session: `playwright-cli open --browser=chrome --headed`.
      - Navigate through each screen defined in the visual test plan.
      - Capture screenshots at each key state: `playwright-cli screenshot --filename=.playwright-cli/screenshots/<requirement-name>/XX-description.png`.
      - Verify visual appearance, layout, and interactive behavior.
      - Document results (pass/fail for each scenario, any visual discrepancies found).
      - Note: `.playwright-cli/` is gitignored. Screenshots are retained locally as verification evidence.
